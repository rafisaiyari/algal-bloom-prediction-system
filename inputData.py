...
class InputDataPage(ctk.CTkFrame):
    ...
    def create_widgets(self):
        # Configure the main frame to expand with window - ENSURE ALL COLUMNS EXPAND
        self.columnconfigure(0, weight=1)
        self.rowconfigure(2, weight=1)  # Make the main data frame row expandable

        inputDatalb = ctk.CTkLabel(self, text="INPUT DATA", font=("Segoe UI", 25, "bold"))
        inputDatalb.grid(row=0, column=0, padx=20, pady=20, sticky="nw")

        # Create filters container frame with proper sizing
        self.filters_container = ctk.CTkFrame(self, fg_color="transparent")
        ...
        deselect_all_btn = ctk.CTkButton(station_filter_controls, text="Deselect All", command=self.deselect_all_stations,
                                       fg_color=self.button_color, hover_color="#18558a")
            ...
            if self.station_vars[station].get():
                if self.pagasa_var.get() and not self.llda_var.get():
                    ...
                    for idx, header in enumerate(self.pagasa_headers):
                        entry = self.entries[station][header]
                        entry.grid(column=idx+1, row=int(self.station_frames[station].grid_info()['row']), padx=10, pady=5, sticky="ew")
                else:
                    # Normal case: use original column positions
                    for header, entry in self.entries[station].items():
                        if header in self.visible_headers:
                            entry.grid(column=self.header_columns[header], row=int(self.station_frames[station].grid_info()['row']), padx=10, pady=5, sticky="ew")
                        else:
                            entry.grid_remove()
