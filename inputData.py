...
class InputDataPage(ctk.CTkFrame):
    ...
    def run_model(self):
        """Run the model and capture all stdout."""
        ...
        def safe_insert(text):
            """Safely insert text into the Text widget via root.after."""
            self.model_output.after(0, lambda t=text: self.model_output.insert("end", t))
        ...
        def run_model_task():
            ...
            class Tee:
                """A class that writes to both buffer and Terminal."""
                def __init__(self, buffer, original_stdout, callback_func=None):
                    self.buffer = buffer
            ...
            tee = Tee(buffer, sys.__stdout__, callback_func=safe_insert)
            ...
            try:
                ...
                self.model_output.after(0, lambda: self.model_output.insert("end", f"\n[ERROR]\n{error_message}"))
            finally:
                output = buffer.getvalue()
                self.model_output.after(0, lambda: self.model_output.insert("end", output))
                self.model_output.after(0, lambda: self.model_output.see("end"))
                self.model_output.after(0, lambda: self.model_output.configure(state="disabled"))
                self.model_output.after(0, lambda: self.run_model_button.configure(state="normal", text="Run Model"))

        thread = threading.Thread(target=run_model_task, daemon=True)
        thread.start()

        '''def update_ui():
            """Update the output text box periodically."""
            try:
                while not output_queue.empty():
                    message = output_queue.get_nowait()
                    self.model_output.insert("end", message)
                    self.model_output.see("end")  # Scroll to end

                if self.model_subprocess_thread.is_alive():
                    self.after(100, update_ui)  # Keep polling
                else:
                    # Done
                    self.run_model_button.configure(state="normal", text="Run Model")  # Final text
                    self.model_output.configure(state="disabled")
            except Exception as e:
                print(f"Error in UI update: {e}")

        # Call the subprocess (no update_ui_func argument unless needed!)
        self.model_subprocess_thread = run_model_subprocess(output_queue, self.current_username, self.user_type, self.audit_logger)
